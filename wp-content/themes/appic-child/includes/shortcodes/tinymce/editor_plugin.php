...
(function(){
	...
	tinymce.create(
		'tinymce.plugins.MyThemeShortcodesChild',
		{
			init: function(z, pliginUrl)
			{
				...
				if ('4' == tinyMCE.majorVersion) {
					jQuery('<style>.mce-ico.mce-i-shortcodes_button{background:url('+icon_url+') left top no-repeat;}></style>')
						.appendTo('head');
console.log('heredfggete');
					z.addButton('shortcodes_button',{
						type:'menubutton',
						title:'Insert Shortcode Child',
						menu:this.getMenuStructureChild()
					});
				...
				z.addCommand('insertThemeShortcode', this._commandInsertShortcodeChild, this);
			},

			_getShortcodesMenuItemsMapChild: function()
			{
			...
			_commandInsertShortcodeChild:function(a,c)
			{
			...
			getMenuStructureChild: function()
			{
				
				var shortcodesMenuItemsChild = this._getShortcodesMenuItemsMapChild();

				var _getSubChild = function(btntext, details){
					if (typeof details == 'string') {
						...
						var r = [],
							t;
						for(t in details) {
							r.push(_getSubChild(t, details[t]));
						};
						return {
							text: btntext,
							//type: 'menubutton',
				...
				var result = [],
					btext;
				for (btext in shortcodesMenuItemsChild) {
					result.push(_getSubChild(btext, shortcodesMenuItemsChild[btext]));
				}
	...
	tinymce.PluginManager.add('MyThemeShortcodesChild',tinymce.plugins.MyThemeShortcodesChild);
})();
